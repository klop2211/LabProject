...
CEllenPlayer::CEllenPlayer(ID3D12Device* pd3dDevice, ID3D12GraphicsCommandList* pd3dCommandList) 
{
	...
	std::ifstream InFile("Model/Ellen.bin", std::ios::binary);
...
CCamera* CEllenPlayer::ChangeCamera(DWORD nNewCameraMode, float fTimeElapsed)
{
	//TODO: 원하는 카메라 시점으로 수정 필요
	DWORD nCurrentCameraMode = (m_pCamera) ? m_pCamera->GetMode() : 0x00;
	if (nCurrentCameraMode == nNewCameraMode) return(m_pCamera);
	switch (nNewCameraMode)
	{
...
void CEllenPlayer::Render(ID3D12GraphicsCommandList* pd3dCommandList, CCamera* pCamera)
{
	DWORD nCameraMode = (pCamera) ? pCamera->GetMode() : 0x00;
	if (nCameraMode == THIRD_PERSON_CAMERA) {
		CGameObject::Render(pd3dCommandList);
